...
CREATE TABLE IF NOT EXISTS users (
    ...
    INDEX idx_users_email (email) COMMENT 'Índice para búsqueda rápida por email (login)'
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- [FIX-1] Columna requerida por el trigger after_task_update
...
--         para mantener coherencia con la política de auditoría.
-- =====================================================
CREATE TABLE IF NOT EXISTS task_comments (
    id          BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    content     TEXT         NOT NULL COMMENT 'Contenido del comentario (puede incluir @menciones)',
    task_id     INT UNSIGNED NOT NULL,
    -- [FIX-3] NULL + SET NULL: el comentario permanece aunque el usuario se elimine
...
CREATE TABLE IF NOT EXISTS task_history (
    ...
    created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'Momento del cambio',
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    INDEX idx_notifications_user_read (user_id, is_read) COMMENT 'Optimiza consulta de notificaciones no leídas.'
...
-- [FIX-1] Ahora usa NEW.updated_by (columna que ya existe en tasks)
-- [FIX-2] Corregido bug en INSERT de due_date:
--         el 4° argumento pasaba NEW.due_date en lugar de 'new_value'
-- -------------------------------------------------
...
BEGIN
    ...
    -- [FIX-2] Cambio de fecha de vencimiento (HU-008)
    -- Bug anterior: 4° argumento era NEW.due_date → corregido a 'new_value' (nombre de columna)
    IF NOT (OLD.due_date <=> NEW.due_date) THEN
        INSERT INTO task_history (task_id, user_id, action, field_changed, old_value, new_value)
        VALUES (NEW.id, NEW.updated_by, 'UPDATE', 'due_date',
    ...
    IF OLD.column_id != NEW.column_id THEN
        INSERT INTO task_history (task_id, user_id, action, field_changed, old_value, new_value)
        VALUES (NEW.id, NEW.updated_by, 'MOVE', 'column_id',
    ...
    IF NOT (OLD.assigned_to <=> NEW.assigned_to) THEN
        INSERT INTO task_history (task_id, user_id, action, field_changed, old_value, new_value)
        VALUES (NEW.id, NEW.updated_by, 'ASSIGN', 'assigned_to',
...
-- =====================================================
-- El campo 'updated_by' en 'tasks' debe ser llenado por el backend
-- en cada sentencia UPDATE, pasando el ID del usuario autenticado.
-- Ejemplo (PHP/Laravel):
--   DB::table('tasks')->where('id', $taskId)->update([
--       'title'      => $newTitle,
--       'updated_by' => Auth::id(),   <-- obligatorio
--   ]);
--
-- NOTA SOBRE <=> (NULL-safe equal):
-- Se reemplazaron las comparaciones compuestas con IS NULL / != por
-- el operador <=> de MySQL, que compara correctamente valores NULL
-- sin necesitar condiciones adicionales. NOT (A <=> B) es TRUE cuando
-- A y B difieren, incluyendo los casos NULL vs no-NULL.
-- =====================================================
-- FIN DEL SCRIPT
-- =====================================================
